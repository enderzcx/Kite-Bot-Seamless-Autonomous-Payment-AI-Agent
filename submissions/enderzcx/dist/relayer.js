...
class RelayerService {
    constructor(rpcUrl, privateKey, tokenAddress, authorizedAgentAddress) {
        this.rpcUrl = rpcUrl;
        this.privateKey = privateKey;
        this.tokenAddress = tokenAddress;
        this.authorizedAgentAddress = authorizedAgentAddress;
        this.provider = new ethers_1.ethers.JsonRpcProvider(rpcUrl);
        this.tokenContract = new ethers_1.ethers.Contract(tokenAddress, [
    ...
    async executeGaslessTransfer(auth) {
        try {
            ...
            const tx = await this.tokenContract.transferWithAuthorization(auth.from, auth.to, auth.value, auth.validAfter, auth.validBefore, auth.nonce, auth.v, auth.r, auth.s);
